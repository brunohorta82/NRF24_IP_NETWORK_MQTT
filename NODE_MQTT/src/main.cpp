...
void loop()
{
  ...
  if (!clientMqtt.connected()) {
    ...
    if(millis()-mqtt_timer > 1000){ //Every 1 second send heartbeat via MQTT
      mqtt_timer = millis();
      clientMqtt.publish("heartbeat",String("{\"name\":\""+String(String("node")+String(ip[3]))+"\",\"heartbeat\":1,\"mesh_address\":"+String(mesh.mesh_address)+",\"_nodeID\":"+String(mesh._nodeID)+",\"voltage\":"+String(voltage)+",\"addrListTop\":"+String(mesh.addrListTop)+",\"ip\":\""+String(ip[0])+"."+String(ip[1])+"."+String(ip[2])+"."+String(ip[3])+"\"}").c_str());
